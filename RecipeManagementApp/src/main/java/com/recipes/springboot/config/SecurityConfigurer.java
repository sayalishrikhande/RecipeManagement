package com.recipes.springboot.config;
...
public class SecurityConfigurer extends WebSecurityConfigurerAdapter {
	...
	/**
	 * Exceptions added to filter for Authentication and Swagger URLs
	 */
	@Override
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests().antMatchers("/recipes/v1/authenticate/").permitAll().antMatchers("/api")
				...
				.antMatchers("/actuator/info").permitAll().antMatchers("/actuator")
				...
				.permitAll().antMatchers("**/recipes/v1/**").permitAll().antMatchers("/api-docs").permitAll().anyRequest().authenticated().and().sessionManagement()
