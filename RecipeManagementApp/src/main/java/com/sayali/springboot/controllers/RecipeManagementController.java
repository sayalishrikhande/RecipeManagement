...
public class RecipeManagementController {
	...
	public ResponseEntity<RecipeResult> getRecipes() {
		...
		ResponseEntity<RecipeResult> recipeResult = new ResponseEntity<RecipeResult>(new RecipeResult(1, null), HttpStatus.OK);

		try {

			recipes = recipeService.getRecipes();

			recipeResult = new ResponseEntity<RecipeResult>(new RecipeResult(200, Messages.SUCCESSFUL.getMessage(), recipes), HttpStatus.OK);

		} catch (Exception e) {

			LOGGER.error("Here in catch " + e.getMessage());
			recipeResult = new ResponseEntity<RecipeResult>(new RecipeResult(1, Messages.UNSUCCESSFUL.getMessage(), null) , HttpStatus.OK);
	...
	public ResponseEntity<RecipeActionResult> getRecipe(@PathVariable("id") int id) {
		...
		ResponseEntity<RecipeActionResult> recipeResult = new ResponseEntity<RecipeActionResult>(new RecipeActionResult(1, null), HttpStatus.OK);
		...
		try {
			...
			if (recipe != null) {
				...
				recipeResult = new ResponseEntity<RecipeActionResult>(new RecipeActionResult(HttpStatus.OK.value(), Messages.SUCCESSFUL.getMessage(), recipe), HttpStatus.OK);
			...
			recipeResult = new ResponseEntity<RecipeActionResult>( new RecipeActionResult(1, Messages.UNSUCCESSFUL.getMessage(), recipe), HttpStatus.OK);
		}
	...
	public ResponseEntity<RecipeActionResult> createRecipe(@RequestBody Recipe recipe) {
		...
		try {
			...
			recipeResult = new ResponseEntity<RecipeActionResult>(new RecipeActionResult(HttpStatus.OK.value(), Messages.SUCCESSFUL.getMessage(),
					createdRecipe),HttpStatus.OK);

		} catch (Exception e) {

			LOGGER.error("In Catch" + e.getMessage());
			recipeResult = new ResponseEntity<RecipeActionResult>(new RecipeActionResult(1, Messages.UNSUCCESSFUL.getMessage(), createdRecipe), HttpStatus.OK);
	...
	public ResponseEntity<RecipeActionResult> updateRecipe(@RequestBody Recipe recipe) {
		...
		try {
			...
			recipeResult = new ResponseEntity<RecipeActionResult>(new RecipeActionResult(HttpStatus.OK.value(), Messages.SUCCESSFUL.getMessage(),
					updateRecipe), HttpStatus.OK);

		} catch (Exception e) {
			LOGGER.error("In catch" + e.getMessage());
			recipeResult = new ResponseEntity<RecipeActionResult>(new RecipeActionResult(1, Messages.UNSUCCESSFUL.getMessage(), updateRecipe), HttpStatus.OK);
